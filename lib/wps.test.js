...
import { testGeoserverUrl } from './config/vitestConfig.ts'
const wpsHelperInstance = new wpsHelper(`${testGeoserverUrl}/ows`);
test('使用wps计算某个图层的要素数量', async () => {
    const countXml = wpsHelperInstance.formatFeatureCountXmlString({
        workspace: "osm",
        workspaceURI: "http://www.openstreetmap.org",
        queryName: "osm:roads",
        cql: "name like 't%'",
    });
    const finishXmlString = wpsHelperInstance.finishXML(countXml);
    const wpsurl = `${testGeoserverUrl}/ows?service=WPS&version=1.0.0`;
    const res2 = await fetchUtil.postXml(wpsurl, finishXmlString)
    if (res2) {
        console.log(res2, "hahahhahahha")
        const jsonRes = JSON.parse(res2);
        console.log(jsonRes, "hahahhahahha")
