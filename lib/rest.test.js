...
test('获取FeatureTypes列表', async () => {
    const res = await restHelperInstance.getFeaturetypesApi("qhd","qhd")
    expect(res).toHaveProperty("featureTypes")
})

test('获取单个FeatureTypes详情', async () => {
    const res = await restHelperInstance.getFeaturetypeInfoApi("qhd","qhd","abcd")
    expect(res).toHaveProperty("featureType")
})
...
test('获取数据资源列表', async () => {
    const res = await restHelperInstance.getResourceDirectoryInfo()
    expect(res).toHaveProperty("ResourceDirectory")
})
...
test('获取用户列表', async () => {
    const res = await restHelperInstance.getUsers()
